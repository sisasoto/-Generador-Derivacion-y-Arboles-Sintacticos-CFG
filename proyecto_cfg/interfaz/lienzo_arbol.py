...
# Paleta pastel (sin cambios de color)
...
class LienzoArbol(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._posiciones: dict[int, tuple[int, int]] = {}
        self.setMinimumSize(400, 300)
        self.setStyleSheet(f"background-color: {_COLOR_FONDO.name()}; border-radius: 8px;")

    def set_arbol(self, raiz: NodoArbol | None) -> None:
        self._raiz = raiz
        self._posiciones = {}
        if raiz is not None:
            self._calcular_posiciones()
            if self._posiciones:
                max_x = max(x for x, _ in self._posiciones.values()) + _RADIO_NODO + _MARGEN
    ...
    def _listar_hojas(self, n: NodoArbol) -> list:
        if n.es_hoja():
            return [n]
        r = []
        for h in n.hijos:
